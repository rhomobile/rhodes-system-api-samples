...
public class NList implements Runnable {
	
	private static final RhoLogger LOG = RhoLogger.RHO_STRIP_LOG ? new RhoEmptyLogger() : 
		new RhoLogger("NList");

	public static RubyModule NListModule;    
	...
	public void run() {
		  LOG.INFO("$$$ Register NList Ruby class");
   	  	  // register Ruby Module
          NListModule = RubyAPI.defineModule("NList");        
          // register Ruby method
          NListModule.getSingletonClass().defineMethod("native_process_string", new RubyOneArgMethod() {
			protected RubyValue run(RubyValue receiver, RubyValue arg0, RubyBlock block) {
				if ( arg0 instanceof RubyString )
				{
					...
					return ObjectFactory.createString(result);
					  ...
					  throw new RubyException(RubyRuntime.ArgumentErrorClass, "in NList.native_process_string: wrong argument type.Should be String");
				}
			}
          });
          NListModule.getSingletonClass().defineMethod("calc_summ", new RubyTwoArgMethod() {
			protected RubyValue run(RubyValue receiver, RubyValue arg1, RubyValue arg2, RubyBlock block) {
				try {
						...
						if (arg1 instanceof RubyInteger) {
							...
							if (arg1 instanceof RubySymbol) {
								...
								x = rv.toInt();
								  ...
								  throw new RubyException(RubyRuntime.ArgumentErrorClass, "in NList.calc_summ: wrong argument type.Should be Fixnum or Integer");
							}
						}
					}
					...
					e.printStackTrace();
					LOG.ERROR("NList.calc_summ failed with exception", e);
