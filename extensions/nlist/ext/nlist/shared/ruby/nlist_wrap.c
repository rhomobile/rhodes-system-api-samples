...
#define SWIG_init    Init_NList
#define SWIG_name    "NList"
...
SWIGINTERN VALUE
_wrap_open_list(int argc, VALUE *argv, VALUE self) {
  ...
  if (!SWIG_IsOK(res2)) {
    SWIG_exception_fail(SWIG_ArgError(res2), Ruby_Format_TypeError( "", "char const *","open_list", 2, argv[1] ));
  }
  arg2 = (char *)(buf2);
  open_list(arg1,(char const *)arg2);
  {
    rho_param_free(arg1);
  }
...
SWIGINTERN VALUE
_wrap_close_list(int argc, VALUE *argv, VALUE self) {
  if ((argc < 0) || (argc > 0)) {
...
extern "C"
#endif
SWIGEXPORT void Init_NList(void) {
  size_t i;
  
  SWIG_InitRuntime();
  mNList = rb_define_module("NList");
  ...
  rb_define_module_function(mNList, "open_list", _wrap_open_list, -1);
  rb_define_module_function(mNList, "close_list", _wrap_close_list, -1);
}
